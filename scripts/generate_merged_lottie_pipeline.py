...
# 展示阶段动效参数（秒定义，按 fps 换算）
# 装饰元素：匀速环形晃动（无速率曲线，不规则环形路径）
...
EASE_LINEAR_O = {"x": [0], "y": [0]}          # 线性出（匀速）
EASE_LINEAR_I = {"x": [1], "y": [1]}          # 线性入（匀速）
...
def _make_anim_layer_style(l, tag, in_start, in_end, out_start, out_end, W, H, F_TOTAL, fps):
    ...
    bounce_dur_f = _s2f(T_BOUNCE, fps)
    t_bounce = min(in_end + bounce_dur_f, out_start - 1)
    layer['_hold_start'] = t_bounce
    layer['_hold_end'] = out_start
    # 保存 asset 尺寸（用于 Stage 4 计算缩放中心）
...
# ═══════════════════════════════════════════════════════════════════════════════
# Stage 4: Hold Animation — 展示阶段动效（装饰元素持续晃动 + 突出元素两次动效）
# ═══════════════════════════════════════════════════════════════════════════════
...
def _parse_highlight_arg(arg):
    ...
    for part in arg.split(','):
        ...
        if not part or ':' not in part:
            continue
        layer_part, anim_type = part.split(':', 1)
        if '_' in layer_part:
            tag, ind = layer_part.split('_', 1)
            tag = tag.lower()
            try:
                ind = int(ind)
...
def _insert_deco_shake(layer, fps, W, H):
    """装饰元素：匀速不规则环形晃动（无速率曲线）
    在 hold 阶段持续做不规则环形运动，每圈采样 N 个点，
    每点半径随机、角度递增，用线性缓动（匀速）连接。
    """
    import math
    hold_start = layer.get('_hold_start', 0)
    hold_end = layer.get('_hold_end', 0)
    if hold_end <= hold_start + 2:
        return
    
    # 获取原始 position（hold 起点时的值）
    pos_kfs = layer['ks']['p']['k']
    base_pos = None
    for kf in reversed(pos_kfs):
        if kf['t'] <= hold_start:
            base_pos = list(kf['s'])
            break
    if base_pos is None:
        base_pos = list(pos_kfs[0]['s']) if pos_kfs else [0, 0, 0]
    
    # 获取原始 rotation
    rot_kfs = layer['ks']['r']['k']
    base_rot = None
    for kf in reversed(rot_kfs):
        if kf['t'] <= hold_start:
            base_rot = kf['s'][0]
            break
    if base_rot is None:
        base_rot = rot_kfs[0]['s'][0] if rot_kfs else 0
    
    new_pos_kfs = []
    new_rot_kfs = []
    t = hold_start
    angle_offset = random.uniform(0, 2 * math.pi)  # 起始角度随机
    loop_count = 0
    
    while t < hold_end - 2:
        # 一圈的时长
        loop_dur = _s2f(random.uniform(*T_DECO_LOOP_DUR), fps)
        ...
        pts = T_DECO_LOOP_PTS
        for i in range(pts + 1):  # pts+1 个点，首尾相同形成闭环
            if i == 0:
                # 起点用原位
                pt_t = t
                ...
                # 中间点：角度递增，半径随机波动
                frac = i / pts
                pt_t = int(t + (loop_end - t) * frac)
                angle = angle_offset + loop_count * 0.7 + frac * 2 * math.pi
                # 半径随机波动（让路径不规则）
    ...
    # 合并并去重时间戳（后出现的覆盖先出现的）
    merged_pos = {kf['t']: kf for kf in pos_kfs + new_pos_kfs}
    merged_rot = {kf['t']: kf for kf in rot_kfs + new_rot_kfs}
    layer['ks']['p']['k'] = sorted(merged_pos.values(), key=lambda k: k['t'])
    layer['ks']['r']['k'] = sorted(merged_rot.values(), key=lambda k: k['t'])

def _insert_highlight_scale(layer, fps):
    """突出元素：缩放两下（100% → 108% → 100% → 108% → 100%）
    以图片中心为缩放中心：同步偏移 position，让视觉中心不动。
    """
    hold_start = layer.get('_hold_start', 0)
    hold_end = layer.get('_hold_end', 0)
    ...
    dur_f = _s2f(random.uniform(*T_HIGHLIGHT_DUR), fps)
    pause_f = _s2f(random.uniform(*T_HIGHLIGHT_PAUSE), fps)
    amp = random.uniform(*T_HIGHLIGHT_SCALE)
    
    # 在 hold 阶段中间开始第一次
    total_need = dur_f * 4 + pause_f * 3  # 四次变化 + 三次停顿
    start_t = max(hold_start, (hold_start + hold_end - total_need) // 2)
    if start_t + total_need > hold_end:
        start_t = hold_start
    
    # 获取基础 scale
    base_scl = layer['ks']['s']['k'] if layer['ks']['s'].get('a') == 0 else [100, 100, 100]
    base_x, base_y = base_scl[0], base_scl[1]
    
    # 获取 anchor 和 position（用于计算缩放中心偏移）
    base_anc = layer['ks']['a']['k'] if layer['ks']['a'].get('a') == 0 else [0, 0]
    ax, ay = base_anc[0], base_anc[1]
    
    # 获取 hold 起点的 position
    pos_kfs = layer['ks']['p']['k']
    base_pos = None
    for kf in reversed(pos_kfs):
        if kf['t'] <= hold_start:
    ...
    if base_pos is None:
        base_pos = list(pos_kfs[0]['s']) if pos_kfs else [0, 0, 0]
    px, py, pz = base_pos[0], base_pos[1], base_pos[2]
    
    # 获取 asset 尺寸（用于计算图片中心）
    aw = layer.get('_aw', 0) or 0
    ah = layer.get('_ah', 0) or 0
    # 如果没有，尝试从 layer 属性获取
    if aw == 0 or ah == 0:
        aw = layer.get('aw', 0) or 0
        ah = layer.get('ah', 0) or 0
    
    # 图片中心（在 asset 坐标系中）
    cx, cy = aw / 2.0, ah / 2.0
    
    # 计算缩放时的 position 偏移：让图片中心不动
    # 元素中心在画布上 = position + (center - anchor) * scale/100
    # 缩放后要让中心不动：position_offset = (anchor - center) * scale/100 * (f - 1)
    # 其中 scale 是原始 scale（base_x），f 是缩放因子
    def calc_pos_offset(f):
        """计算缩放因子为 f 时的 position 偏移"""
        ox = (ax - cx) * (base_x / 100.0) * (f - 1.0)
        oy = (ay - cy) * (base_y / 100.0) * (f - 1.0)
        return ox, oy
    
    # 生成 scale 和 position 关键帧
    scl_kfs = []
    new_pos_kfs = []
    t = start_t
    # 静止 → 放大 → 回 → 放大 → 回
    states = [1.0, amp, 1.0, amp, 1.0]
    for i, state in enumerate(states):
        ox, oy = calc_pos_offset(state)
        scl_kfs.append(_kf(int(t), [base_x * state, base_y * state, 100], EASE_STD_I, EASE_STD_O))
        new_pos_kfs.append(_kf(int(t), [px + ox, py + oy, pz], EASE_STD_I, EASE_STD_O))
        if i < len(states) - 1:
            t += dur_f if i % 2 == 0 else pause_f
...
def stage_hold_anim(keyframes_out, deco_items=None, highlight_items=None):
    """Stage 4: 展示阶段动效
    
    参数:
        deco_items: [(tag, ind), ...] 装饰元素列表
        highlight_items: [(tag, ind, type), ...] 突出元素列表，type 为 'scale' 或 'rotate'
    """
    ...
    for layer in out_layers:
        ...
        # 装饰元素晃动
        for d_tag, d_ind in (deco_items or []):
            if tag == d_tag and orig_ind == d_ind:
                _insert_deco_shake(layer, fps, meta['w'], meta['h'])
                applied_deco.append((tag, orig_ind))
                break
        
        # 突出元素动效（统一只保留缩放，以图片中心为缩放中心）
        for h_tag, h_ind, h_type in (highlight_items or []):
            if tag == h_tag and orig_ind == h_ind:
                _insert_highlight_scale(layer, fps)
                applied_highlight.append((tag, orig_ind, 'scale'))
                break
    ...
    return {
        ...
        'layers': out_layers,
        'deco_applied': applied_deco,
        'highlight_applied': applied_highlight,
    }


def stage_hold_anim_check(hold_out):
    """Stage 4 自检：检查关键帧时间戳递增"""
    for l in hold_out['layers']:
        for prop in ['p', 'r', 's']:
            ks = l['ks'].get(prop, {})
            if ks.get('a') == 1:
                kfs = ks['k']
                for i in range(1, len(kfs)):
                    if kfs[i]['t'] <= kfs[i-1]['t']:
                        _fail(4, f"{l.get('nm', '')}.ks.{prop} t={kfs[i]['t']} <= t={kfs[i-1]['t']}")
    _ok(4, f'展示阶段动效: {len(hold_out["deco_applied"])} 个装饰元素, {len(hold_out["highlight_applied"])} 个突出元素')


# ═══════════════════════════════════════════════════════════════════════════════
# Stage 5: Assemble — ind 编号 + parent remap + 循环锚点 + refId 验证
# ═══════════════════════════════════════════════════════════════════════════════

def stage_assemble(hold_out):
    """Stage 5: 组装最终 Lottie JSON"""
    meta = hold_out['meta']
    F_TOTAL = hold_out['timeline_params']['F_TOTAL']
    out_layers = deepcopy(hold_out['layers'])
    assets = hold_out['assets']
...
def stage_assemble_check(output, loop_fixed):
    """Stage 5 自检：refId 完整性 + 循环锚点"""
    asset_ids = {a['id'] for a in output['assets']}
    bad = [(l.get('nm'), l.get('refId')) for l in output['layers']
           if l.get('refId') and l['refId'] not in asset_ids]
    if bad:
        _fail(5, f'refId 引用缺失: {bad}')
    assert loop_fixed > 0, '循环锚点数为 0，可能没有动画层'
    _ok(5, f'组装完成 {len(output["layers"])}层 {len(output["assets"])}assets 循环锚点{loop_fixed}处')
...
def main():
    ...
    # 解析 --deco 和 --highlight
    deco_items = _parse_deco_arg(args.deco)
